...
The kernel chooses which CPU core is responsible for processing a request is determined through Interrupt Request (IRQ) Affinity. IRQ Affinity is an assignment of a given queue to a given CPU and the overall process is called IRQ Affinity pinning.
...
Secondly, the 99th percentile latency has decreased significantly. With six or fewer threads, we are now able to achieve the required QoS. Additionally, the 99th percentile latency reaches a minimum with 6 threads providing the best combination of low latency and highest throughput. This is matches our expectation.
...
In our benchmarks, we have observed that group size does not have any impact on the overall performance of Memcached. We have used 147 simultaneous connections in our benchmark, however, a smaller number of connections with higher throughput may be required to effectively exploit the group size. The benchmark setup does not allow us to generate a high enough load from a single connection and therefore we are unable to replicate the results obtained by Blake and Saidi \cite{blake54does}.
...
In this section, we consider the impact of packet coalescing on the performance of Memcached. Packet coalescing is used to group packets together in order to limit the number of interrupts fired. It is effectively packet batching in the network interface controller.

In this benchmark, we consider two scenarios - one where packet coalescing is disabled and the other with packet coalescing enabled. We configure our workload to use our best performing configuration - with IRQ pinned.

\begin{figure}[h]
    \includegraphics[width=\textwidth]{./res2/m_coalesce.png}
    \caption{Memcached Packet Coalescing on and off}
    \label{fig:m_coalesce}
\end{figure}

Figure \ref{fig:m_coalesce} shows the performance of Memcached in terms of 99th percentile latency, operations per second and CPU utilization with packet coalescing disabled. We can see that with packet coalescing disabled, we achieve a lower throughput as well as higher latency. The difference in throughput is significant, Memcached is only capable of processing 468k requests a second with coalescing off as opposed to 557k request per second with coalescing enabled. Additionally, disabled packet coalescing leads to more CPU cycles spent processing software interrupts by 4\%.

We can conclude that packet coalescing greatly improves the performance of Memcached. It allows Memcached to deliver nearly 90k requests more at lower 99th percentile latency.
