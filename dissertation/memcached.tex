...
Interestingly, neither throughput nor 99th percentile latency change between 1 and 2 threads. We would expect the throughput to increase with more threads, however, this appears to not be the case. When Memcached runs with only 1 thread, there is no need for resource locking and therefore lock contention is eliminated. We can argue that with 2 threads, lock contention introduces an overhead which does not outweigh the benefit of increased parallelization.
...
Let us consider individual CPU utilization breakdown with 6 threads. Figure \ref{fig:m_threads_irq_cpu_individual} outlines the utilization categories. We can observe that all cores are nearly 100\% utilized as well as all cores participate in software interrupt processing.
